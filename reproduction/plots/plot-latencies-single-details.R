...
do.filter <- function(df, do.print=FALSE, all.groups = TRUE) {
  df <- df %>% 
    filter(o_masters_replicas_id == '3-1') %>%
...
ylimbaseline <- mapply("-", range(data$latency_ms[data$facet == "Baseline"]), ylim_expansion)
ylimfailover1 <- mapply("-", range(data$latency_ms[(data$facet == "CP Full" | data$facet == "CP Part") & data$node_name == "Gr. 1"]), ylim_expansion)
ylimfailover2 <- mapply("-", range(data$latency_ms[(data$facet == "CP Full" | data$facet == "CP Part") & data$node_name == "Gr. 2"]), ylim_expansion)
#ylimpatchpush <- mapply("-", range(data$latency_ms[data$facet == "LP Pull"]), ylim_expansion)
#ylimpatchpull <- mapply("-", range(data$latency_ms[data$facet == "LP Push"]), ylim_expansion)
ylimpatch <- mapply("-", range(data$latency_ms[data$facet == "LP Push" | data$facet == "LP Pull"]), ylim_expansion)
...
plot <- ggplot() +
  ...
  facetted_pos_scales(
    y = list(
      facet == "Baseline" & node_name == "Gr. 1" ~ scale_y_continuous(limits = ylimbaseline),
      facet == "Baseline" & node_name == "Gr. 2" ~ scale_y_continuous(limits = ylimbaseline),
      facet == "Baseline" & node_name == "Gr. 3" ~ scale_y_continuous(limits = ylimbaseline),
      
      facet == "CP Part" & node_name == "Gr. 1" ~ scale_y_log10(limits = ylimfailover1, breaks = c(0.1, 1, 10, 100, 1000), labels=c("", "1", "10", "100", "")),
      facet == "CP Part" & node_name == "Gr. 2" ~ scale_y_continuous(limits = ylimfailover2),
      
      facet == "CP Full" & node_name == "Gr. 1" ~ scale_y_log10(limits = ylimfailover1, guide="none"),
      facet == "CP Full" & node_name == "Gr. 2" ~ scale_y_continuous(limits = ylimfailover2, guide="none"),
      
      facet == "LP Pull" & node_name == "Gr. 1" ~ scale_y_continuous(limits = ylimpatch),
      facet == "LP Pull" & node_name == "Gr. 2" ~ scale_y_continuous(limits = ylimpatch),
      facet == "LP Pull" & node_name == "Gr. 3" ~ scale_y_continuous(limits = ylimpatch),
      
      facet == "LP Push" & node_name == "Gr. 1" ~ scale_y_continuous(limits = ylimpatch, guide="none"),
      facet == "LP Push" & node_name == "Gr. 2" ~ scale_y_continuous(limits = ylimpatch, guide="none"),
      facet == "LP Push" & node_name == "Gr. 3" ~ scale_y_continuous(limits = ylimpatch, guide="none")
    )
  )
...
plot <- add.plot.failover.restart.rectangles(plot, action.data.failover.restart, ymin=0)
